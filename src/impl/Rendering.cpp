...
const Color& Texture::At(int x, int y) const { return At(Index32(x, y)); }
      ...
      Color& Texture::At(int index32)       { assert_on_host(); MarkForUpdate(); return *(Color*)(Pixels() + Index(index32)); }
const Color& Texture::At(int index32) const { assert_on_host();                  return *(Color*)(Pixels() + Index(index32)); }
...
Mesh& Mesh::Add(AttribName name, int instancedStride, int forceRepeat, const r<Buffer>& buffer)
{
...
int gl_element_type_size(Buffer::ElementType type)
{
	switch (type)
	{
		case Buffer::_u8:  return sizeof(u8);
		case Buffer::_u32: return sizeof(u32);
		case Buffer::_f32: return sizeof(f32);
	}
