...
namespace Input
{
    ...
    // these two should be more generic
    // or split the serialization code into another file
    ...
    InputAxisBuilder& InputAxisBuilder::Map(InputCode code, vec2 weight)
    {
        ...
        ctx->Mapping.emplace(code, m_name);
        return *this;
    }
    ...
    AxisGroupBuilder::AxisGroupBuilder(AxisGroup* axis, const InputName& name)
        : InputAxisSettingsBuilder (&axis->settings)
        , m_axis                   (axis)
        , m_name                   (name)
    {}
    ...
    AxisGroupBuilder& AxisGroupBuilder::Map(const InputName& name)
    {
        ...
        for (const auto& c : ctx->Axes[name].components)
            ctx->Mapping[c.first] = m_name;
	...
	InputAxisBuilder CreateAxis(const InputName& name)
	{
        ...
        if (_AxisExists(name))
            ...
            axis = &ctx->Axes.emplace(name, InputAxis{}).first->second;
        
        return InputAxisBuilder(axis, name);
	}
	...
	AxisGroupBuilder CreateGroupAxis(const InputName& name)
	{
        ...
        if (_GroupAxisExists(name))
            ...
            group = &ctx->GroupAxes.emplace(name, AxisGroup{}).first->second;
        
        return AxisGroupBuilder(group, name);
	}
    ...
    const InputName& GetMapping(InputCode code)
    {
        ...
        return itr != ctx->Mapping.end() ? itr->second : _default;
    }
